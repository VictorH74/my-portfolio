...
export class Hexagon {
    ...
    private update() {
        ...
        const xConstraint =
            prevXPos - this.size < -450 ||
            prevXPos + this.size > this._canvasWidth + 450;
        const yConstraint =
            prevYPos - this.size < -450 ||
        ...
        const hexagonXBeforeCenter =
            xPos >= centerAreaLeft - centerAreaLeft / 3 &&
            xPos <= centerAreaLeft;
        const hexagonXAfterCenter =
            xPos <= centerAreaRight + centerAreaLeft / 3 &&
            xPos >= centerAreaRight;
    ...
    draw() {
        const [r, g, b] = this.calculatePercentageRGBValue(
            this._colorAndSizePercentage
        );
        this.update();
        ...
        for (let i = 1; i <= 6; i += 1)
            this._canvasCtx.lineTo(
                this._position.x +
                    calculateSize(this._colorAndSizePercentage, 80, this.size) *
