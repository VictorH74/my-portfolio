...
export class Trie {
    ...
    public insert(words: string[]) {
        words.forEach((word) => {
            ...
            for (let i = 0; i < word.length; i++) {
                ...
                if (i === word.length - 1) node.children[word[i]].isEnd = true;
    ...
    public findsByPrefix(prefix: string): string[] {
        ...
        for (let i = 0; i < prefix.length; i++) {
            if (!currentNode.children[prefix[i]]) break;

            if (
                i === prefix.length - 1 &&
                currentNode.children[prefix[i]].isEnd
            )
                words.push(prefix);
    ...
    public findFirstByPrefix(prefix: string): string | undefined {
        let currentNode = this.node;
        for (let i = 0; i < prefix.length; i++) {
            if (!currentNode.children[prefix[i]]) return undefined;
