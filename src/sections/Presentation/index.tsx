...
const textLeading = "sm:leading-[4rem] leading-[3rem] max-[430px]:leading-[2.1rem]"
  ...
  return values.map((v, i) => {
    if (i === 1) {
      return (
        <h1
          key={i}
          className={`emphasy text-main-color text-6xl ${textLeading} max-sm:text-5xl max-[430px]:text-[2.2rem] ${notoSans.className} `}
        >
          {v}
        </h1>
      );
    ...
    return (
      <h2
        key={i}
        className={`mr-2 text-5xl max-sm:text-[2.5rem] ${textLeading} max-[430px]:text-[1.7rem] ${notoSans.className} `}
      >
        {v}
      </h2>
    );
  ...
  return (
    <section className={`h-[85vh] mt-[15vh] relative`}>
      ...
      <KeyboardDoubleArrowDownIcon
        ...
        sx={{ fontSize: 50, color: "var(--mainColor)" }}
      />
    </section>
  );
};
